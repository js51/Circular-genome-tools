...
def model(G, n, signed=True, model_type = MODEL.ONE_AND_TWO_REGION_INVERSIONS):
	...
	if model_type == MODEL.TWO_REGION_INVERSIONS:
		return sorted(__all_inversions_model(G, n, signed, num_regions = 2))
	elif model_type == MODEL.ONE_AND_TWO_REGION_INVERSIONS:
		return sorted(__all_inversions_model(G, n, signed, num_regions = 1) + __all_inversions_model(G, n, signed, num_regions = 2))
